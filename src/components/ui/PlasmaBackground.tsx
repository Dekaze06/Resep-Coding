  ...
  useEffect(() => {
    ...
    const compileShader = (type: number, source: string) => {
      ...
      if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
        console.error('Plasma shader compile error:', gl.getShaderInfoLog(shader));
    ...
    if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
      console.error('Plasma program link error:', gl.getProgramInfoLog(program));
      return;
    }
    ...
    gl.uniform4f(uShapeLoc, 1.50, 0.48, 0.50, 0.00);     // scale, intensity, paramA, warp
    gl.uniform4f(uSurfaceLoc, 2.40, 0.92, -0.50, 1.00);  // detail, contrast, brightness, saturation
    gl.uniform4f(uFinishLoc, 3.04, 0.61, 0.016, 0.18);   // hue, vignette, blur, grain
    gl.uniform4f(uTransformLoc, 7.0, 0.00, 0.32, 0.0);   // seed, rotation, drift, OKLab toggle
    gl.uniform4f(uSpaceLoc, 0.00, 0.00, 0.0, 0.0);       // offset xy, mouse zw
    gl.uniform4f(uCursorLoc, 0.0, 2.0, 0.65, 0.30);      // presence = 0.0 (Cursor: off)
    ...
    const render = () => {
      ...
      gl.useProgram(program);
      // u_scene = vec4(canvas width, canvas height, seconds * 0.59, 4.0)
      gl.uniform4f(uSceneLoc, canvas.width, canvas.height, elapsedSeconds * 0.59, 4.0);
