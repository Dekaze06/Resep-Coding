...
export function GenerativeArtScene({ className = "" }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    const material = new THREE.ShaderMaterial({
      ...
      fragmentShader: `
        ...
        void main() {
            ...
            vec3 finalColor = (color * diffuse * 0.4 + color * fresnel * 0.7) * 0.7;
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      const x = ((e.clientX - rect.left) / currentMount.clientWidth) * 2 - 1;
      const y = -((e.clientY - rect.top) / currentMount.clientHeight) * 2 + 1;
      ...
      if (material.uniforms.pointLightPos) {
...
export function AnomalousMatterHero({
  ...
  return (
    <section
      role="banner"
      className="relative w-full h-screen bg-[#09090b] text-zinc-100 overflow-hidden"
    >
      <Suspense fallback={<div className="w-full h-full bg-[#09090b]" />}>
        <GenerativeArtScene />
      </Suspense>

      <div className="absolute inset-0 bg-gradient-to-t from-[#09090b] via-[#09090b]/70 to-transparent z-10" />
      ...
      <div className="relative z-20 flex flex-col items-center justify-end h-full pb-20 md:pb-32 text-center">
        <div className="max-w-3xl px-4 animate-fade-in-long">
          ...
          <p className="mt-6 max-w-xl mx-auto text-base leading-relaxed text-zinc-400">
            {description}
          </p>
        </div>
      </div>
