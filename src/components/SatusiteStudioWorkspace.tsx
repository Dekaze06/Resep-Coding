...
export default function SatusiteStudioWorkspace() {
  const [genMode, setGenMode] = useState<"fullstack" | "frontend">("fullstack");
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      ...
      role: "agent",
      agentName: "AI Agent",
      text: "Halo! Pilih mode Fullstack atau Frontend di bawah, lalu ketikkan ide website yang ingin Anda buat.",
      timestamp: "Siap",
    }
  ]);
  ...
  useEffect(() => {
    try {
      ...
      if (qMode === "frontend" || qMode === "fullstack") {
        setGenMode(qMode);
      }
      ...
      if (qId && store.projects[qId]) {
        ...
        setProjectName(name);
        setHasGenerated(true);
        handleSendPrompt(qPrompt, name, newId);
      }
    } catch (err) {
  ...
  const handleSendPrompt = async (promptToSend?: string, customName?: string, customId?: string) => {
    ...
    const isFull = genMode === "fullstack";
    ...
    try {
      ...
      setMessages(prev => [
        ...prev,
        {
          ...
          role: "agent",
          agentName: "AI Agent",
          text: `Pemberitahuan: ${err.message}. Silakan perbaiki instruksi Anda.`,
          timestamp: new Date().toLocaleTimeString("id-ID", { hour: "2-digit", minute: "2-digit" })
        }
      ]);
  ...
  // Quick Action Pills for ongoing conversation
  const quickActions = [
    { label: "+ Tombol WhatsApp", prompt: "Tambahkan tombol pemesanan WhatsApp mengambang di pojok kanan bawah yang langsung membuka chat WA." },
    { label: "+ Form Kontak", prompt: "Tambahkan bagian formulir kontak / konsultasi dengan input nama, no telepon, dan pesan." },
    { label: "+ Galeri Foto", prompt: "Tambahkan galeri showcase foto produk dengan efek hover rapi dan filter kategori." },
    { label: "+ FAQ Accordion", prompt: "Tambahkan bagian tanya jawab (FAQ) interaktif yang bisa diklik buka-tutup." },
    { label: "+ Tabel & Pencarian", prompt: "Tambahkan tabel data lengkap dengan kolom pencarian / filter real-time dan tombol ekspor CSV." },
    { label: "+ Dark/Light Toggle", prompt: "Tambahkan tombol switch mode gelap dan terang pada bagian header navigasi." }
  ];
  ...
  const previewSrcDoc = useMemo(() => {
    ...
    if (code.includes("</head>")) {
      return code.replace("</head>", `${scrollbarCss}</head>`);
    }
    return scrollbarCss + code;
  }, [code]);
      ...
      <div className={`bg-zinc-900/60 rounded-xl focus-within:bg-zinc-900/80 transition-colors ${isCentered ? "border border-zinc-800/80 shadow-lg" : ""}`}>
          ...
          <div className="flex items-center gap-1.5">
            ...
            <div className="relative mode-dropdown-container">
              ...
              >
                ...
                <span className="capitalize text-[11px]">{genMode}</span>
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between">
                        ...
                        {genMode === "fullstack" && <Check className="w-3.5 h-3.5 text-blue-400" />}
                      </div>
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between">
                        ...
                        {genMode === "frontend" && <Check className="w-3.5 h-3.5 text-blue-400" />}
                      </div>
                      <p className="text-[10px] text-zinc-400 leading-snug mt-0.5">
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            ...
            className="w-6 h-6 rounded-md bg-zinc-800 hover:bg-zinc-700 text-white flex items-center justify-center transition-all disabled:opacity-20 cursor-pointer"
        ...
        <div className="flex-1 flex flex-col md:flex-row overflow-hidden relative">
          ...
          <aside className={`w-full ${showCanvas ? "md:w-[35%]" : "md:w-full max-w-3xl mx-auto"} border-r border-zinc-800/60 bg-zinc-950 flex flex-col shrink-0 overflow-hidden z-20 transition-all duration-500`}>
            ...
            <div className="px-3 py-2 border-b border-zinc-800/40 flex items-center justify-between text-xs">
              ...
              <span className="text-[10px] text-zinc-500 font-mono">35% Panel</span>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-3 space-y-3">
              {messages.map((m) => (
                <div
                  key={m.id}
                  className={`flex flex-col ${m.role === "user" ? "items-end" : "items-start"} animate-fade-in-up`}
                >
                  ...
                  >
                    ...
                    {m.hasCodeUpdate && (
                      <button
                        ...
                        className="mt-2 text-[10px] text-blue-400 hover:text-blue-300 font-medium transition-colors"
                      >
              ...
              {isGenerating && (
                <div className="bg-zinc-900/50 rounded-lg p-3 space-y-1.5 animate-fade-in-up">
                  <div className="flex items-center gap-2 text-xs text-blue-400">
                    ...
                    <span className="font-medium">Memproses website...</span>
                  </div>
            ...
            {/* Quick action pills */}
            <div className="px-3 py-1.5 border-t border-zinc-800/40 bg-zinc-950">
              <div className="flex gap-1 overflow-x-auto no-scrollbar">
                {quickActions.map((qa, i) => (
                  <button
                    ...
                    disabled={isGenerating}
                    className="shrink-0 px-2 py-0.5 rounded-md bg-zinc-900/60 hover:bg-zinc-800 text-[10px] text-zinc-400 hover:text-white transition-colors"
                  >
                    {qa.label}
                  </button>
                ))}
