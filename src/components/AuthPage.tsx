...
export default function AuthPage() {
  ...
  const handleGoogleCredentialResponse = async (response: any) => {
    ...
    setIsLoading(true);
    setErrorMsg('');
    setSuccessMsg('Memverifikasi akun Google Anda...');
    ...
    try {
      ...
      if (res.ok && data.success && data.user) {
        ...
        setSuccessMsg(`Selamat datang, ${data.user.name}! Mengalihkan ke portal...`);
        setTimeout(() => {
    ...
    try {
      if (authMode === 'register') {
        ...
        if (res.ok && data.success && data.user) {
          localStorage.setItem('satusite_auth_user', JSON.stringify(data.user));
          localStorage.setItem('satusite_auth_token', data.token);
          setSuccessMsg('Pendaftaran akun berhasil! Mengalihkan ke portal...');
          setTimeout(() => {
            window.location.href = '/portal';
          }, 600);
        } else {
